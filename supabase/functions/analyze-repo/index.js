...
function calculateScores(repoData) {
...
serve(async (req) => {
  ...
  try {
    const { owner, repo, searchQuery, content } = await req.json();
    
    console.log("Request parameters:", { owner, repo, searchQuery, content });
    ...
    if (content) {
      ...
      if (match && match.length >= 3) {
        ...
        const repoData = await fetchRepoData(extractedOwner, extractedRepo);
